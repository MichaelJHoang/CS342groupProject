...
public class Game {
	...
	public Game( Scanner s, int nChars) {
		...
		for( int i = 0; i < numCharacters; i++) {
			...
			else if(p.equalsIgnoreCase("OGRE")) {
				ch = new Ogre(s, placeID);
				npcs.addElement(ch);
			}
			else if( p.equalsIgnoreCase("MERCHANT")) {
				ch = new Merchant(s, placeID);
				npcs.addElement(ch);				
			}
			else if( p.equalsIgnoreCase("DOG")) {
				ch = new DOG(s, placeID);
				npcs.addElement(ch);
			}
		...
		if( nChars > numCharacters) {
			...
			for( int i = 0; i < difference; i++) {
				...
				name += idAsString;
				ch = new Player( id, name , "Manually created player character", Place.getFirstPlace().getID() );
				id++;
				players.addElement(ch);
				numPlayers++;
				numOfCharacters++;
		...
		//if num of characters from file was 0
			...
			ch = new Player( 1, "Generic Player", "Manually created player character", Place.getFirstPlace().getID());
			players.addElement(ch);
			numPlayers++;
			numOfCharacters++;
		}
